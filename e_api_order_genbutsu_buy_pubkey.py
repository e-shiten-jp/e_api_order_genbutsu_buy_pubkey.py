...
S_SIZYOU_C = '00'      # 11.市場。  00:東証   現在(2021/07/01)、東証のみ可能。
S_CONDITION = '0'     # 13.執行条件。  0:指定なし、2:寄付、4:引け、6:不成。指し値は、0:指定なし。
S_ORDER_PRICE = '145'  # 14.注文値段。  *:指定なし、0:成行、上記以外は、注文値段。小数点については、関連資料:「立花証券・e支店・API、REQUEST I/F、マスタデータ利用方法」の「2-12. 呼値」参照。
S_ORDER_SURYOU = '800' # 15.注文数量。
...
def func_order_genbutsu_buy(
    ...
    dic_req_item = {
        ...
        # 固定パラメーターセット
        'sZyoutoekiKazeiC': str_sZyoutoekiKazeiC,   # 8.譲渡益課税区分    1：特定  3：一般  5：NISA     ログインの返信データで設定済み。 
